document.addEventListener("DOMContentLoaded", () => {
  ...
  // username 은 영대소문자와 언더바(_)를 사용할수 있으며 4 ~ 20글자 까지가능
  ...
  btn_join.addEventListener("click", async () => {
    ...
    if (!userNameExp.test(username.value)) {
      alert(
        "USERNAME 은 영문대소문자, 숫자, _로 4~20글자만 가능합니다"
      );
      username.select();
      return false;
    }
    ...
    if (!password.value) {
      alert("PASSWORD 는 반드시입력하세요");
      password.select();
      return false;
    }

    if (!passwordExp.test(password.value)) {
      alert("비밀번호는 8자리이상 영문 숫자 특수문자만 가능합니다");
      password.select();
      return false;
    }

    if (!re_password.value) {
      alert("PASSWORD 확인은 반드시입력하세요");
      re_password.select();
      return false;
    }
