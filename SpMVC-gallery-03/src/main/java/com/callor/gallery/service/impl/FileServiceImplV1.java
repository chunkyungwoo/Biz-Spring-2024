...
public class FileServiceImplV1 implements FileService{
	...
	/*
	 * 편의상 FileServiceImplV1 을 FileServiceImplV2 에서 상속하였다
	 ...
	 * 이럴경우 변수를 private 로 선언해버리면 V2 에서 다시 생성자를
	 * 통하여 upLoadPath 변수를 주입받아야 한다
	...
	public String fileUp(MultipartFile file) throws Exception {
		...
		if(!path.exists()) {
			/*
			 * path.mkdis(),path.mkdirs()
			 * 폴더를 생성하는 method 
			 * mkdir() : 한개의 폴더만 생성하기 : app/upload 폴더를 생성할경우
			 * 		app 폴더가 없으면 오류가 발생
			 * mkdirs() : 여러 경로의 폴더 생성하기 : app/upload 폴더를 생성할경우
			 * 		app 폴더, upload 폴더를 모두 생성
			 */
			path.mkdirs();
		}
		String uuid = UUID.randomUUID().toString();
		// 원래의 파일 이름 앞에 uuid 를 부착하여 업로드할 파일이름 생성
	...
	public List<ImageVO> filesUp(MultipartHttpServletRequest files) throws Exception {
		...
		/*
		 * view 의 form input[type='file'] tag 의 name(=files) 속성을 통하여
		 * 파일 리스트 추출하기
		 */
		...
		for(MultipartFile file : fileList) {
			...
			ImageVO vo = ImageVO.builder().i_up_image(resultName).i_origin_image(originName).build();
			resultFiles.add(vo);
		}
